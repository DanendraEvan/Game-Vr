...
// kasih seed random biar maze konsisten
...
Array.prototype.repeat= function(what, L){
   ...
   // Variabel global untuk hitung kertas
...
function paintMaze(maze){
 ...
 // Print walls
 function makeWall(width, height, pos, rot, repeat){
   ...
   wall.setAttribute('material','src: #asset_wall;side: double;repeat: '+repeat);
   scene.appendChild(wall);
 }
 makeWall(3*maze_width, 3, '0 1.5 '+(3*((maze_height)*.5)), '0 0 0', maze_width+' 1');
 makeWall(3*maze_width, 3, '0 1.5 -'+(3*((maze_height)*.5)), '0 0 0', maze_width+' 1');
 makeWall(3*maze_height, 3, (3*((maze_width)*.5))+' 1.5 0', '0 90 0', maze_height+' 1');
 makeWall(3*maze_height, 3, (-3*((maze_width)*.5))+' 1.5 0', '0 90 0', maze_height+' 1');
 ...
 // === BUAT 3 KERTAS ===
 function makePaper(id, pos){
   var paper = document.createElement("a-plane");
   paper.setAttribute("id", id);
   paper.setAttribute("src", "#asset_paper");
   paper.setAttribute("width", "0.5");
   paper.setAttribute("height", "0.5");
   paper.setAttribute("rotation", "0 180 0");
   paper.setAttribute("position", pos);
   paper.setAttribute("static-body", "");
   paper.setAttribute("material", "transparent:true; alphaTest:0.5; side:double");

   paper.addEventListener("click", function () {
     paper.parentNode.removeChild(paper);
     collectedPapers++;
     var remaining = Math.max(0, 3 - collectedPapers);
 ...
 // Lokasi 3 kertas (bebas bisa kamu atur lagi)
 makePaper("paper1", "-19 1.6 -17");
 makePaper("paper2", "-13 1.6 -17");
 makePaper("paper3", "-7.6 1.6 -17");
